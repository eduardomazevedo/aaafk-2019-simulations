...
1. Setup
	...
	b. In order to run any analysis in ./analysis files follow the instructions on the readme.txt in ./aass folder.
...
------------------------------------------------------------------------------------------------------------------------------
   ...
   a. The directories are organized as follows:
	...
	i.	./aass (Advanced Asynchronous Simulation System) includes .sh files for the bash application
	ii.	./analysis contains all the exercises.
	iii.	./classes contains main classes file, simulation.m.
	...
	vii.	./log keeps the log files for analysis
	viii.	./output keeps figures, tables and some functions
...
In order to generate the results, one should run all the analyses through AASS system. AASS system takes spec.m files within the analysis folder and runs a set of simulations. Relevant analysis’s results will be saved ./data with in the directory of the analysis. ./read-data has the relevant pieces of code to read the raw data and create interim structs and matrices. ./generate-output has the relevant pieces of code to read the interim outputs from ./read-data and create relevant structs and matrices for the paper.
